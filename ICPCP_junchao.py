...
class Workflow():     
        ...
        for (key, value) in data.items():
            if isinstance(value, list) :
                if key == 'nodes' :
                    ...
                    #print "nodes:",value
                    for node in value:
                        self.G.add_node(self.vertex_num, order = node['order'], name = node['name'], est = -1, eft = -1, lst = -1, lft =-1)
                        self.vertex_num += 1 
                if key == 'links' :
                    #print "links:",value
    ...
    def assign_parents(self, i):  
        while (self.has_unassigned_parent(i)):  
            ...
            if not assigned_vm == -2 and not assigned_vm == -1:
                    self.G.node[pcp[len(pcp)-1]]['eft'] = self.G.node[pcp[len(pcp)-1]]['est'] + self.p_table[self.assigned_list[pcp[len(pcp)-1]], pcp[len(pcp)-1]]
                    self.G.node[pcp[0]]['lst'] = self.G.node[pcp[0]]['lft'] - self.p_table[self.assigned_list[pcp[0]], pcp[0]]
    ...
    def assign_path(self, pcp): 
        ...
        if chosen_instance == None:    #no existing instance for the pcp
            ...
            self.G.node[head_pcp]['eft'] = self.G.node[head_pcp]['est'] + self.p_table[self.assigned_list[head_pcp], head_pcp]
            est = self.G.node[pcp[len(pcp)-1]]['est']
    ...
    def print_instances(self):
        ...
        if tot_idle>0:
            print "\nTotal cost for "+str(self.vertex_num)+" nodes: "+str(total_cost)+" with tot idle="+str(tot_idle)
        else:
    ...
    def graphCheckEST(self):
        for n in xrange(0, self.vertex_num) :
            ...
            if maxest>self.deadline :
              print "\n**** Wrong EST max: "+"EST("+self.G.node[n]["name"]+")="+str(self.G.node[n]["est"])+" and "+"EST("+self.G.node[dominant_parent]["name"]+")="+str(maxest)
              return -1
            elif self.G.node[n]["est"] < maxest :
                print "\n**** EST mismatch: "+"EST("+self.G.node[n]["name"]+")="+str(self.G.node[n]["est"])+" < "+"EST("+self.G.node[dominant_parent]["name"]+")="+str(maxest)
                return -1
    ...
    def graphCheckLFT(self):
        for n in xrange(0, self.vertex_num) :
            ...
            # node with no children has LFT equals deadline
            if minlft<0 :
                print "\n**** Negative LFT : "+"LFT("+self.G.node[n]["name"]+")="+str(self.G.node[n]["lft"])+" and "+"LFT("+self.G.node[dominant_child]["name"]+")="+str(minlft)
                return -1
            elif self.G.node[n]["lft"] > minlft :
                print "\n**** LFT mismatch: "+"LFT("+self.G.node[n]["name"]+")="+str(self.G.node[n]["lft"])+" > "+"LFT("+self.G.node[dominant_child]["name"]+")="+str(minlft)
                return -1
        ...
        for i in xrange(0, len(self.instances)) :
            ...
            if len(task_list)>1:
                for j in xrange(0, len(task_list)-1):
                    ...
                    if idle_time>0 :
                        tot_idle += idle_time
                        idles += "\n Instance["+str(i)+"] constains idle time: "+"EST("+self.G.node[task_list[j+1]]["name"]+")-EFT("+self.G.node[task_list[j]]["name"]+")>0"
        print idles
        return tot_idle
